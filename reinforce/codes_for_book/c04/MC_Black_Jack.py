...
from utils import greedy_pi, str_key, set_dict, get_dict


A = ["继续叫牌","停止叫牌"] 
Prediction_by_fixed_policy = False
...
class Player(Person):
    def __init__(self, policy = None,name="", role = "", display = False, A = A) -> None:
...
class Arena():
    ...
    def __init__(self, display = None, A = A) -> None:
    ...
    def play_game(self, dealer, player):
        ...
        #玩家说话 points < 12 自动发牌
        while True:
            s = get_state(dealer=dealer, player=player)  # 获取对仗状态信息
...
def greedy_pi(A, s, Q, a):
    '''依据贪婪选择，计算在行为空间A中，状态s下，a行为选中的几率
    考虑多个行为的价值相同的情况
    '''
    max_q, a_max_q = -float('inf'), []
    for a_opt in A:  # 统计后续状态行为的最大价值以及到达该状态行为价值的行为（可能不止一个）
...
if not Prediction_by_fixed_policy:
    ...
    print("总共玩了{}局".format(player.total_learning_times))
