...
def epsilon_greedy_policy(A, s, Q, epsilon, show_random_num=False):
    ...
    for i in range(m):
        ...
        if rand_value < 0:
            return A[i]
    return A[pis.index(max(pis))]


def epsilon_greedy_black_jack_policy(A, s, Q, epsilon, show_random_num=False):
    _, first_dealer_card, player_points, useable_ace = s
    s1 = first_dealer_card, player_points, useable_ace
    ...
    if player_points >= 21:
        ...
        return epsilon_greedy_policy(A, s1, Q, epsilon, show_random_num)
